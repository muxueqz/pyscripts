...
def ssh_con(host, sshoptions, port, user, passwd, cmd=""):
#    print sigwinch_passthrough('test','test')
    columns=115
    lines=38
    ssh_program = 'ssh -o "StrictHostKeyChecking=no"'
    spawn = ('%s %s %s -p %s -l %s %s') % (ssh_program, sshoptions, host, port, user, cmd)
    print user, host, cmd
...
def main():
    ...
    if len(sys.argv) >= 1:
        ...
        ssh_con(host, opts.sshopt, port, user, passwd, cmd)
